...
import { Home, Package, Users, RefreshCw, Search, Upload, GraduationCap, ChevronLeft, ChevronRight, User, LogOut, FileText, History, Trash2, Plug, Settings } from 'lucide-react';
...
const menuItems = [{
  ...
  icon: Upload,
  label: 'Publicar Anúncios',
  path: '/publicar-ofertas',
  active: false
}, {
  icon: RefreshCw,
  label: 'Atualização de Estoque',
  path: '/atualizacao-estoque',
  active: false
}, {
  icon: Trash2,
  label: 'Deletar Anúncios',
  path: '/deletar-ofertas',
  active: false
}, {
  icon: History,
  label: 'Histórico',
...
const ExpandedLogo = () => <svg xmlns="http://www.w3.org/2000/svg" width="80" height="50" viewBox="0 0 60 37.5" preserveAspectRatio="xMidYMid meet" className="h-14 w-auto">
    <defs>
    ...
    <g>
      ...
      <g clipRule="nonzero" clipPath="url(#9f4b6143b8)">
        ...
        fill: '#ffffff',
        fillOpacity: 1
      }} d="M 53.925781 27.804688 C 53.925781 28.480469 53.378906 29.027344 52.703125 29.027344 L 48.183594 29.027344 C 47.511719 29.03125 46.964844 28.480469 46.964844 27.804688 C 46.964844 27.128906 47.511719 26.585938 48.183594 26.585938 L 52.699219 26.585938 C 53.378906 26.585938 53.925781 27.128906 53.925781 27.804688 Z M 53.925781 27.804688 " />
      </g>
    </g>
  </svg>;

// Componente SVG customizado para quando o sidebar estiver colapsado
const CollapsedLogo = () => <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 375 374.999991" preserveAspectRatio="xMidYMid meet" className="w-8 h-8">
    <defs>
    ...
    <g>
      ...
      <g clipRule="nonzero" clipPath="url(#a403a4aaf5)">
        ...
        fill: '#ffffff',
        fillOpacity: 1
      }} d="M 266.339844 343.484375 C 266.339844 358.359375 254.273438 370.359375 239.320312 370.359375 L 139.285156 370.359375 C 124.328125 370.460938 112.261719 358.359375 112.261719 343.484375 C 112.261719 328.609375 124.328125 316.609375 139.285156 316.609375 L 239.214844 316.609375 C 254.273438 316.609375 266.339844 328.609375 266.339844 343.484375 Z M 266.339844 343.484375 " />
      </g>
    </g>
  </svg>;
export const DraggableSidebar = ({
  ...
  return <TooltipProvider>
      <div className="relative">
        <div ref={sidebarRef} className="bg-slate-900 text-white transition-all duration-300 flex flex-col fixed left-0 top-0 h-screen z-40" style={{
        width: `${sidebarWidth}px`
      }}>
          {/* Header */}
          <div className="p-4 flex items-center justify-center border-b border-slate-700 min-h-[73px]">
            {sidebarWidth > 120 ? <div className="flex items-center justify-center w-full">
                <ExpandedLogo />
          ...
          <nav className="flex-1 p-4 space-y-2">
            ...
            const menuButton = <Link key={index} to={item.path} className={cn("flex items-center p-3 rounded-lg cursor-pointer transition-colors relative", isActive ? "bg-blue-600 text-white" : "hover:bg-slate-700 text-slate-300", sidebarWidth <= 120 && "justify-center")}>
                  <item.icon size={sidebarWidth <= 120 ? 24 : 20} />
                  {sidebarWidth > 120 && <>
                      <span className="ml-3 font-medium">{item.label}</span>
                      {item.badge && <span className="ml-auto bg-green-400 text-green-900 text-xs px-2 py-1 rounded-full font-medium">
                          {item.badge}
                        </span>}
                    </>}
                </Link>;
          ...
          <div className="p-4 border-t border-slate-700 space-y-2">
            ...
            {sidebarWidth <= 120 ? <Tooltip>
              ...
              </Tooltip> : <div className="flex items-center p-3 rounded-lg space-x-3">
                <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center">
                  <span className="text-sm font-medium">
                    {selectedUser?.user?.charAt(0).toUpperCase() || 'G'}
                  </span>
            ...
            {sidebarWidth <= 120 ? <Tooltip>
                <TooltipTrigger asChild>
                  <Link to="/integracoes" className="flex items-center w-full p-3 rounded-lg cursor-pointer hover:bg-slate-700 transition-colors text-slate-300 hover:text-white justify-center">
                    <Plug size={24} />
                  </Link>
                </TooltipTrigger>
                <TooltipContent side="right" className="bg-slate-800 text-white border-slate-600">
              ...
              </Tooltip> : <Link to="/integracoes" className="flex items-center w-full p-3 rounded-lg cursor-pointer hover:bg-slate-700 transition-colors text-slate-300 hover:text-white space-x-3">
                <Plug size={20} />
            ...
            {sidebarWidth <= 120 ? <Tooltip>
              ...
              </Tooltip> : <button className="flex items-center w-full p-3 rounded-lg cursor-pointer hover:bg-slate-700 transition-colors text-slate-300 hover:text-white space-x-3">
                <Settings size={20} />
            ...
            {sidebarWidth <= 120 ? <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={handleLogout} className="flex items-center w-full p-3 rounded-lg cursor-pointer hover:bg-red-600 transition-colors text-slate-300 hover:text-white justify-center">
                    <LogOut size={24} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="right" className="bg-slate-800 text-white border-slate-600">
              ...
              </Tooltip> : <button onClick={handleLogout} className="flex items-center w-full p-3 rounded-lg cursor-pointer hover:bg-red-600 transition-colors text-slate-300 hover:text-white space-x-3">
                <LogOut size={20} />
        ...
        <button onClick={onToggle} onMouseDown={handleToggleMouseDown} className="fixed z-50 p-2 bg-slate-800 hover:bg-slate-700 text-white rounded-r-lg transition-all duration-300 border-r border-t border-b border-slate-600 shadow-lg cursor-move" style={{
